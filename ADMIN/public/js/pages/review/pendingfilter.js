    ...
    $.ajax({
        ...
        { 
            $.getJSON("https://crowd-src.herokuapp.com/domain", function (data) 
            {
                if(c==1)
                {
                    $("#getdomain").append("<li class='use active'><a>All</a></li>");
                    for (var i=0;i<data.TOTAL_NO_OF_DOMAINS;i++)
                    {
                        $("#getdomain").append("<li class='use'><a>"+data.DOMAIN_DETAILS[i].domain+"</a></li>");
                    }
                    var title;
                    ...
                    {      
                        ...
                        //API TO GET ALL THE PROJECTS FOR A PERTICULAR DOMAIN
                            ...
                            $.getJSON("https://crowd-src.herokuapp.com/upload/pending"+title, function (data) 
                            {
            ...
            $.getJSON('https://crowd-src.herokuapp.com/category', function (data) {
                if(c==1)
                {
                    for (var i=0;i<data.TOTAL_NO_OF_CATEGORY;i++){
                        $("#getdept").append("<li class='cat'><a>"+data.CATEGORY_DETAILS[i].category+"</a></li>");
                    }
